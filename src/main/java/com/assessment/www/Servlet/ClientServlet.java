...
public class ClientServlet extends HttpServlet {
    ...
    private void showProfile(HttpServletRequest request, HttpServletResponse response, User user) throws ServletException, IOException {
        // 获取用户发布的视频
        List<com.assessment.www.po.Video> userVideos = videoService.getUserVideos(user.getId(),user);
    ...
    private void searchVideos(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        List<com.assessment.www.po.Video> videos = videoService.searchVideos(user,keyword, page, Constants.PAGESIZE);
        boolean hasKeyword = keyword != null && !keyword.trim().isEmpty();
        int totalCount = hasKeyword ? videoService.getTotalVideoCount(user) : videoService.getPublicVideoCount(user);
    ...
    private void coinVideo(HttpServletRequest request, HttpServletResponse response, User user) throws IOException {
        ...
        try {
            ...
            if (!success) {
                ...
                if (userCoins < amount) {
                    response.getWriter().write("{\"success\": false, \"error\": \"投币失败，硬币不足\"}");
                } else {
