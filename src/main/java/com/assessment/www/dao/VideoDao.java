...
public interface VideoDao {
    ...
    //扯video的数据
    List<Video> getVideosByAuthorId(int authorId,User user) throws Exception;

    // 获取作者发布的已审核通过的视频
    List<Video> getApprovedVideosByAuthorId(int authorId) throws Exception;

    // 获取我关注的用户发布的视频（按时间倒序）
    List<Video> getFollowingVideos(int userId, User user, int page, int pageSize) throws Exception;

    // 获取我关注的用户发布的视频总数
    int getFollowingVideoCount(int userId, User user) throws Exception;

    // 获取当前用户可见的已审核通过视频
    List<Video> getApprovedVisibleVideos(User user) throws Exception;

    // 个人视频
    List<Video> getAllVideos(User user) throws Exception;

    // 获取所有视频（包括所有状态）
    List<Video> getAllVideosIncludingPending() throws Exception;

    List<Video> searchByKeyword(String keyword, User user) throws Exception;
    ...
    int sendCommentWithPhoto(int videoId, int userId, String text, String photoBase64, float time, Integer parentId) throws Exception;
    ...
    // 获取用户点赞过的视频列表
    List<Video> getLikedVideosByUserId(int userId, int page, int pageSize) throws Exception;
