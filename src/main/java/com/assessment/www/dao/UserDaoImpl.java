...
public class UserDaoImpl implements UserDao {
    ...
    public List<Integer> getFollowingIds(int userId) {
        ...
        String sql = "SELECT user_id FROM user_follows WHERE follower_id = ? ORDER BY time_follow DESC";
        try (Connection conn = utils.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, userId);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public List<Integer> getFollowerIds(int userId) {
        List<Integer> ids = new ArrayList<>();
        String sql = "SELECT follower_id FROM user_follows WHERE user_id = ? ORDER BY time_follow DESC";
        try (Connection conn = utils.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, userId);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public int getNewUsersCountThisMonth() throws Exception {
        String sql = "SELECT COUNT(*) FROM users WHERE YEAR(create_time) = YEAR(NOW()) AND MONTH(create_time) = MONTH(NOW())";
        try (Connection conn = utils.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
    ...
    //更新关注数和粉丝数
    private void updateFollowCounts(int myId, int targetId, int change) throws Exception {
        // 更新目标用户的粉丝数
        String sql1 = "UPDATE users SET follower_count = follower_count + ? WHERE id = ?";
        // 更新我的关注数
        String sql2 = "UPDATE users SET following_count = following_count + ? WHERE id = ?";
        try (Connection conn = utils.getConnection();
             PreparedStatement pstmt1 = conn.prepareStatement(sql1);
             PreparedStatement pstmt2 = conn.prepareStatement(sql2)) {
            // 更新目标用户的粉丝数
            pstmt1.setInt(1, change);
            pstmt1.setInt(2, targetId);
            pstmt1.executeUpdate();
            // 更新我的关注数
            pstmt2.setInt(1, change);
            pstmt2.setInt(2, myId);
            pstmt2.executeUpdate();
        }
    }

    //更新用户状态
    private int updateStatus(int id, int status) throws Exception {
