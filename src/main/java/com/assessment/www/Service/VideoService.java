...
public interface VideoService {
    ...
    // 获取用户发布的视频列表
    List<Video> getUserVideos(int userId,User user);

    // 获取用户发布的已审核通过的视频列表
    List<Video> getApprovedVideosByAuthorId(int userId);

    // 获取我关注用户发布的动态视频
    List<Video> getFollowingVideos(int userId, User user, int page, int pageSize);

    // 获取我关注用户发布的动态视频总数
    int getFollowingVideoCount(int userId, User user);

    // 获取所有视频（分页）
    List<Video> getAllVideos(User user, int page, int pageSize);

    // 获取所有视频（包括所有状态）
    List<Video> getAllVideosIncludingPending();

    // 获取视频总数
    int getTotalVideoCount(User user);

    // 获取当前用户可见的已审核视频总数
    int getPublicVideoCount(User user);

    // 根据关键字搜索视频
    List<Video> searchVideos(User user, String keyword, int page, int pageSize);

    // 根据关键字搜索视频，支持可见范围过滤
    List<Video> searchVideos(String keyword, int page, int pageSize, User user);
    ...
    // 发送带照片的评论
    boolean sendComment(int videoId, int userId, String text, String photoBase64, float time, Integer parentId);
    ...
    // 获取用户点赞视频总数
    int getUserLikedCount(int userId);
