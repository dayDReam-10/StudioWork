...
<main class="main">
    <div class="top">
        <h1>我的收藏</h1>
        <div class="tip">可在此快速取消收藏</div>
    </div>

    <%
        List<Video> favoriteVideos = (List<Video>) request.getAttribute("favoriteVideos");
        ...
        <div class="card" id="fav-<%= v.getId() %>">
            ...
            <div class="info">
                ...
                <div class="actions">
                    ...
                    <button class="btn del" onclick="unfavorite(<%= v.getId() %>)">取消收藏</button>
                </div>
            </div>
        </div>
        <% } %>
    ...
    <div class="pager">
        <% if (currentPage > 1) { %>
        <a href="${pageContext.request.contextPath}/user/favorites?page=<%= currentPage - 1 %>">上一页</a>
        <% } %>
        <% for (int i = 1; i <= totalPages; i++) {
               if (i == currentPage) { %>
        <span class="active"><%= i %></span>
        <%     } else { %>
        <a href="${pageContext.request.contextPath}/user/favorites?page=<%= i %>"><%= i %></a>
        <%     }
           } %>
        <% if (currentPage < totalPages) { %>
        <a href="${pageContext.request.contextPath}/user/favorites?page=<%= currentPage + 1 %>">下一页</a>
        <% } %>
    </div>
    <% } %>

    <% } else { %>
    <div class="empty">你还没有收藏视频。去视频详情页点一下“收藏”吧。</div>
    <% } %>
</main>
...
<script>
    ...
    function unfavorite(videoId) {
        if (!confirm("确定取消收藏这个视频吗？")) return;
        fetch("${pageContext.request.contextPath}/video/unfavorite", {
            method: "POST",
            ...
            if (!d) return;
            if (d.success) {
                var card = document.getElementById("fav-" + videoId);
                if (card) card.remove();
                if (!document.querySelector("#favGrid .card")) {
                    location.reload();
                }
            } else {
                alert(d.message || "取消收藏失败");
            }
        }).catch(function () {
            alert("请求失败");
        });
